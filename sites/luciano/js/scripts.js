...
function temp_script () {
	if (!document.querySelector('.smena_house')) return;
	document.querySelector('.smena_house').addEventListener('click', ()=>{
		document.querySelector('.js--plan_map').classList.add('active');	   
	});
	
	document.querySelector('.js--plan_btn_close').addEventListener('click', ()=>{
		setTimeout(()=>{document.querySelector('.js--plan_map').classList.remove('active');	}, 0);    	
	});
}

temp_script();

function filter_list(array = [], floor = 1){
	var reset = false;
	
	if($('.filter_form').hasClass('filter_list')){
		var count     = 0;
		var rooms     = [];
		var floor_min = parseInt($('input[name=floor_min]').val());
		var floor_max = parseInt($('input[name=floor_max]').val());
		var plo_min   = parseInt($('input[name=plo_min]').val());
		var plo_max   = parseInt($('input[name=plo_max]').val());
		var free      = 0;
		
		var floor_min_default = parseInt($('input[name=floor_min]').data('floor-min'));
		var floor_max_default = parseInt($('input[name=floor_min]').data('floor-max'));
		
		var plo_min_default  = parseInt($('input[name=plo_min]').data('plo-min'));
		var plo_max_default  = parseInt($('input[name=plo_max]').data('plo-max'));
		
		$('input[name=f_rooms]').each(function(i, e){
			if($(this).is(':checked')){
				rooms.push(parseInt($(this).val()));
			}
		});
		
		if($('input[name=only_free]').is(':checked')){
				free = 1;
		}
		...
		$('.table_list tbody tr').each(function(i, e){
				...
				if(
					...
					floor_max >= $(this).data('floor') &&
					floor_min <= $(this).data('floor') &&
					
					plo_max >= $(this).data('plo') &&
					plo_min <= $(this).data('plo') &&
					
					rooms.indexOf($(this).data('rooms')) != -1
				){
					
					
					if(free == 1 && $(this).data('status') == 1){
						$(this).removeClass('none');
						count += 1;
					}else if(free == 1 && $(this).data('status') == 0){
						$(this).addClass('none');
					}else if(free == 0){
						$(this).removeClass('none');
						count += 1;
		...
		$('.count_search span').text(count);
	}
	
	if($('.filter_form').hasClass('filter_plan')){
		var count     = 0;
		var rooms     = [];
		var floor_min = parseInt($('input[name=floor_min]').val());
		var floor_max = parseInt($('input[name=floor_max]').val());
	...
	if($('.filter_form').hasClass('filter_gen')){
		var rooms     = [];
		var plo_min   = parseInt($('input[name=plo_min]').val());
		var plo_max   = parseInt($('input[name=plo_max]').val());
		var free      = 0;
		...
		$.each(array, function(i, e){
			var plo = parseInt(e.plo);
			if(
				plo_max >= plo &&
				plo_min <= plo &&
				rooms.indexOf(parseInt(e.rooms)) != -1
			){
				console.log(e.status);
				if(free == 1 && e.status == 1){
					$('.page_paln polygon[data-id='+i+']').removeClass('none');
				}else if(free == 1 && e.status != 1){
					$('.page_paln polygon[data-id='+i+']').addClass('none');
				}else if(free == 0){
					$('.page_paln polygon[data-id='+i+']').removeClass('none');
				}
			}else{
...
function isMyScriptLoaded(url) {
	if (!url) url = "https://api-maps.yandex.ru/2.1/?apikey=fb0303a9-9b32-47eb-92d2-86375edfbc3a&lang=ru_RU";
...
function comparePage(){
	...
	if(window.location.pathname == '/compare/'){
		...
		if(localStorage.getItem('compare2') != null){
			compare = JSON.parse(localStorage.getItem('compare2'));
			if(compare.length > 0){
				empty = 1
			}
		}
		...
		if(empty == 1){
			$.get('/ajax_w/', {list: localStorage.getItem('compare2')}, function(data){
				$('.text-content').append(data);
			});
		}else{
			$('.text-content').append('<p>Вы не добавили апартаменты для сравнения</p>');
		}
	}
}

function compareCheck(){
	if($('.add.addWhishList').length > 0){
		var id = $('.add.addWhishList').attr('data-id');
		id = parseInt(id);
		var compare = [];
		if(localStorage.getItem('compare2') != null){
			compare = JSON.parse(localStorage.getItem('compare2'));
			if ($.inArray(id, compare) != -1){
				$('.add.addWhishList').addClass('active');
			}
		}
	}
}
function compareAddDel(a){
	var compare = [];
	if(localStorage.getItem('compare2') != null){
		compare = JSON.parse(localStorage.getItem('compare2'));
	}
	...
	if(type == 'add'){
		...
		if(compare != null){
			compare = $.grep(compare, function(value){
				return value != id;
			});
		}
		...
		if(path == '/compare/'){
			...
			if(a.parent().hasClass('compare_tpl')){
				a.parent().remove();
			}
			if($('.compare_items .compare_tpl').length === 0){
	...
	localStorage.setItem('compare2', JSON.stringify(compare));
...
$(document).ready(function(){
	...
	$(document).on('click', '.add.addWhishList', function() {
		compareAddDel($(this));
	});
	
	setTimeout(function(){
		if(isMyScriptLoaded() == false){
			setTimeout(function(){$(document.body).append('<script src="https://api-maps.yandex.ru/2.1/?apikey=fb0303a9-9b32-47eb-92d2-86375edfbc3a&lang=ru_RU"></script>')}, 500);
			setTimeout(function(){$(document.body).append('<script src="/js/map.js?v=1"></script>')}, 2000);
		}
	}, 1500);
	...
	$('.smena_house_zoom').click(function(){$('.smena_house').addClass('active');$('body').addClass('hidden')});
	$('.smena_house_close').click(function(){$('.smena_house').removeClass('active');$('body').removeClass('hidden')});
	$('.smena_house img').click(function(){$('.smena_house').removeClass('active');$('body').removeClass('hidden')});
	...
	$('.compare_tpl__item').mouseover(function(){
		if($(this).hasClass('active')) return;
		var index = $(this).data('index');
		$('.compare_tpl__item').removeClass('active');
		
		$('.compare_tpl__item').each(function(){
	...
	$('.compare_next').click(function(event) {
		event.preventDefault();
		var l = "+=320px";
		if(widthSize <= 780) var l = "+=220px";
		
		if(widthSize < 440){
			var s = $('.compare_items .compare_tpl').width();
			s += 1;
			l = "+=" + s;
		}
		
		$('.compare_items').animate({
			scrollLeft: l
		}, "slow");
	});
	
	$('.compare_prev').click(function(event) {
		event.preventDefault();
		var l = "-=320px";
		if(widthSize <= 780) var l = "-=220px";
	...
	$('form.form_default').on('submit', function(event){
		event.preventDefault();
		var form = $(this);
		$('.errors_form').text('');
		...
		$.ajax({
			...
			success: function(json){
				...
				console.log(json);
				if(json == null){
					$('.errors_form').text('Что-то пошло не так');
					return false;
				}
				if(json.status == 'error'){
					$('.errors_form').text(json.message);
					return false;
				}else if(json.status == 'success'){
					
					$('.thanks_form').removeClass('none');
					form.trigger('reset');
					$('.form_default').trigger('reset');
					$('.form').removeClass('active');
					$('.form_name').text($('.form_name').data('default'));
	...
	$('.slider_nav span').click(function(){
		...
		$('.slider_item[data-id='+id+']').removeClass('active').addClass('closeSlider');
		setTimeout(() => $('.slider_item[data-id='+id+']').removeClass('closeSlider'), 1500)
		
		$('.slider_item').each(function(){
			if(parseInt($(this).data('id')) == play){
				$(this).addClass('active');
			}
		});
	});
	...
	if (typeof rooms !== "undefined") {
		$.each(rooms, function(i, e){
			if(e.status == 2){
				$('polygon[data-id='+i+']').addClass('close');
			}
			if(e.status == 3){
				$('polygon[data-id='+i+']').addClass('bron');
			}
		});
	}
	...
	$('.page_paln polygon').mouseover(function(){
		if(widthSize < 781){
			return
		}
		var uid = $(this).data('id');
		if(parseInt(uid) > 0){
			$('.plan_this').addClass('active');
			$('.plan_this_img').append('<img src="/assets/images/plans/'+uid+'.png" alt="">');
			$('#ploGen').text(rooms[uid].plo);
			$('#uid').text(uid);
			if(rooms[uid].show_price == 1 && rooms[uid].status == 1){
				$('.this_price strong').html(rooms[uid].price).parent().removeClass('none');
			}else{
				$('.this_price').addClass('none');
			}
			$('#count_rooms').text(rooms[uid].rooms);
			if(rooms[uid].status == 1){
				$('#statusgen').text('Свободно');
			}else if(rooms[uid].status == 2){
				$('#statusgen').text('Продано');
			}else{
				$('#statusgen').text('Забронировано');
			}
			
		}
	});
	
	$('.page_paln polygon').mouseout(function(){
		if(widthSize < 781){
			return
		}
		$('.plan_this').removeClass('active');
		$('.plan_this_img').removeClass('active').empty();
	});
	
	$('.close_desc_plan').click(function(){
		$('.plan_this').removeClass('active');
		$('.plan_this_img').removeClass('active').empty();
	});
	
	$('.page_paln polygon').click(function(){
		var uid = $(this).data('id');
		var link = '/apartaments/'+uid+'/';
		if(parseInt(uid) > 0){
			if(widthSize < 781){
				...
				$('.plan_this').find('a.btn_type1').attr('href', link);
	...
	$(document).on("mousemove", function(event) {
		var div = $('.tg43 polygon');
		if (div.is(event.target) && div.length > 0 ) {
			var y = event.pageY - 87 - 75;
			var x = event.pageX + 15;
			$('.trd').css({'top': y+'px', 'left': x+'px', 'display': 'block'});
		}else{
			$('.trd').css('display', 'none');
		}
	});
	
	$('.tg43 polygon[data-floor=4]').mouseover(function(){
		$('.trd .et strong').text(4);
		var rooms = $(this).data('rooms');
		$('.trd .st strong').text(rooms);
		
	});
	
	$('.tg43 polygon[data-floor=3]').mouseover(function(){
		$('.trd .et strong').html(3);
		var rooms = $(this).data('rooms');
		$('.trd .st strong').text(rooms);
	});
	
	$('.tg43 polygon[data-floor=2]').mouseover(function(){
		$('.trd .et strong').text(2);
		var rooms = $(this).data('rooms');
		$('.trd .st strong').text(rooms);
	});
	
	$('.tg43 polygon[data-floor=1]').mouseover(function(){
		$('.trd .et strong').text(1);
		var rooms = $(this).data('rooms');
		$('.trd .st strong').text(rooms);
	});
	
	
	$('.open_osn_form').click(function(){
		var text = $(this).text();
		$('.form').addClass('active');
		$('body').addClass('hidden');
		$('input[name=uid]').val($(this).data('uid'));
		$('input[name=zag]').val(text);
	});
	
	$('.open_ipoteka_form').click(function(){
		var text = $(this).text();
		$('.form').addClass('active');
		$('body').addClass('hidden');
		$('.form_name').text(text);
	...
	$('.form_close_thanks').click(function(){
		$('.thanks_form').addClass('none');
		$('body').removeClass('hidden');
	});
	...
	$('.table_list tbody tr').mouseover(function(){
		var img = $(this).data('plan');
		$('.plan_this').addClass('active').append('<img src="'+img+'" alt="">');
	});
	
	$('.table_list tbody tr').mouseout(function(){
		$('.plan_this').removeClass('active').empty();
	});
	...
	$('.filter_plan input').on('change', function(){
		filter_list();
	});
	
	$('.filter_gen input').on('change', function(){
		var floor = $('.all_etaj').find('a.active').find('strong').text();
		filter_list(rooms, floor);
	});
	...
	let min_plo = 0;
	let max_plo = 10;
	if($('input[name=plo_min]').length > 0){
		min_plo = parseInt($('input[name=plo_min]').data('plo-min'));
		max_plo = parseInt($('input[name=plo_max]').data('plo-max'));
	}
	$( ".pole.one" ).slider({
		range: true,
		animate: true,
		min: min_plo,
		max: max_plo,
		values: [ min_plo, max_plo ],
		slide: function( event, ui ) {
			$('.sf1').html(ui.values[ 0 ]);
			$('input[name=plo_min]').val(ui.values[ 0 ]);

			$('.sf2').html(ui.values[ 1 ]);
			$('input[name=plo_max]').val(ui.values[ 1 ]);
			filter_list();
		}
	});
	...
	let min_floor = 0;
	let max_floor = 4;
	if($('input[name=floor_min]').length > 0){
		min_floor = parseInt($('input[name=floor_min]').data('floor-min'));
		max_floor = parseInt($('input[name=floor_max]').data('floor-max'));
	}
	$( ".pole.two" ).slider({
		range: true,
		animate: true,
		min: min_floor,
		max: max_floor,
		values: [ min_floor, max_floor ],
		slide: function( event, ui ) {
			$('.sf3').html(ui.values[ 0 ]);
			$('input[name=floor_min]').val(ui.values[ 0 ]);

			$('.sf4').html(ui.values[ 1 ]);
			$('input[name=floor_max]').val(ui.values[ 1 ]);
			filter_list();
		}
	});
...
$(document).ready(function() {
	...
	$('.owl-carousel').each(function(){
		...
		if($(window).width() < 770){
			var items = $(this).data('mobile-items');
		}
		var nav      = $(this).data('nav');
		var dots     = $(this).data('dots');
		var autoplay = $(this).data('autoplay');
		if($(this).data('carouselInitialized') != ''){
			carouselInitialized = $(this).data('carouselInitialized');
		}
		$(this).owlCarousel({
			navigation:nav,
			items:items,
	...
	$('.thumb_image').click(function(){
		var index = $(this).data('index');
		$('.big_images .owl-carousel').trigger('to.owl.carousel', index);
