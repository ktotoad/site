...
if(document.querySelector("#block")) {
	...
	block.addEventListener("click", function (event) {
		if(event.target.matches('.item') || event.target.closest(".item")) {
			...
			let list = item.closest("[data-spollers]");
			let price = item.dataset['p'];
			let width = item.dataset['w'];
			let height = item.dataset['h'];
			let zmm = item.dataset['z'];
			...
			block.querySelector("[data-image-price] span").textContent = numberWithSpaces(price);
			block.querySelector("[data-image-w] span").textContent = width;
			block.querySelector("[data-image-h] span").textContent = height;
			console.log(zmm);
		}
	});
}