...
function initSliders() {
	...
	if (document.querySelector('.projects-detail-slider')) {
		new Swiper('.projects-detail-slider', {
			observer: true,
			observeParents: true,
			slidesPerView: 1,
			spaceBetween: 10,
			...
			pagination: {
				el: '.projects-detail__pagintaion',
				clickable: true,
			},
			navigation: {
				nextEl: ".projects-detail-slider__next",
				prevEl: ".projects-detail-slider__prev",
			},
		});
	}
}
...
if(document.querySelector('.tab-list')) {
	...
	document.querySelector('.tab-list').addEventListener('click', e => {
		...
		if(e.target.classList.contains('tab-button') || e.target.closest('.tab-button')) {
			...
			let mySwiper = e.target.qclosest('section').querySelector('.swiper').swiper;
			mySwiper.update();
		}
	});
}
